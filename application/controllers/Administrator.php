...
// Controller Modul Pegawai
	...
	function layanan(){
		...
		$data['rss'] = $this->model_layanan->list_layanan_rss();
        $data['iden'] = $this->db->query("SELECT * FROM identitas ORDER BY id_identitas DESC LIMIT 1")->row_array();
        $this->load->view(template().'/tss',$data);
	...
	function tambah_layanan(){
		cek_session_admin();
		if (isset($_POST['submit'])){
			$this->model_layanan->list_layanan_tambah();
			redirect('administrator/layanan');
		}else{
			$data['tag'] = $this->model_layanan->tag_layanan();
			$data['record'] = $this->model_layanan->kategori_layanan();
			$data['users'] = $this->model_users->users();
			$this->template->load('administrator/template','administrator/mod_layanan/view_layanan_tambah',$data);
		}
	}
	...
	function edit_layanan(){
		...
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			$this->model_layanan->list_layanan_update();
			redirect('administrator/layanan');
		}else{
			$data['tag'] = $this->model_layanan->tag_layanan();
			$data['record'] = $this->model_layanan->kategori_layanan();
			$data['rows'] = $this->model_layanan->list_layanan_edit($id)->row_array();
			$data['users'] = $this->model_users->users();
			$this->template->load('administrator/template','administrator/mod_layanan/view_layanan_edit',$data);
		}
	}
	...
	function delete_layanan(){
		$id = $this->uri->segment(3);
		$this->model_layanan->list_layanan_delete($id);
		redirect('administrator/layanan');
	}
	...
	function kategoriproduct(){
		cek_session_admin();
		$data['record'] = "halo";
		$this->template->load('administrator/template','administrator/mod_product/view_product',$data);
	}
	...
	function tambah_kategorilayanan(){
		cek_session_admin();
		if (isset($_POST['submit'])){
			$this->model_layanan->kategori_layanan_tambah();
			redirect('administrator/kategorilayanan');
		}else{
	...
	function edit_kategorilayanan(){
		cek_session_admin();
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])){
			$this->model_layanan->kategori_layanan_update();
			redirect('administrator/kategorilayanan');
		}else{
			$data['rows'] = $this->model_layanan->kategori_layanan_edit($id)->row_array();
	...
	function delete_kategorilayanan(){
		$id = $this->uri->segment(3);
		$this->model_layanan->kategori_layanan_delete($id);
		redirect('administrator/kategorilayanan');
	}
